...
export default function MemberLimitsWidget({
  ...
  const handleInviteSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setInviteError(res.error || t.memberNotFound);
      }
    } catch (err) {
      setInviteError(t.connectionFailed);
    } finally {
      setIsInviting(false);
    }
  };
  ...
  const handleAddExternalSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        setExtSuccess(t.addExternalSuccess);
        setExternalName("");
      } else {
        setExtError(res.error || t.failedToAdd);
      }
    } catch (err) {
      setExtError(t.connectionFailed);
    } finally {
  ...
  const handleUpgradeSubmit = async (e: React.FormEvent, externalId: string) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setUpgradeError(res.error || t.mergeFailed);
      }
    } catch (err) {
      setUpgradeError(t.connectionFailed);
    } finally {
    ...
    <div className="flex items-center justify-between w-[calc(100%-12px)]">
      <div className="flex flex-col leading-tight truncate text-left">
        <span className="text-white text-xs font-extrabold">
          {t.memberBudgetLimits}
        </span>
        <span className="text-[10px] text-slate-400 font-normal truncate">
          {t.setPersonalThresholds}
        </span>
      </div>
  ...
  return (
    <CollapsibleSection
      title={titleNode}
      headerIcon={<Users size={14} className="text-indigo-400 shrink-0" />}
      defaultCollapsed={isMemberLimitsCollapsed}
    ...
    >
      <div className="pt-2 space-y-4 text-left">
        {onInviteUser && (
          ...
          >
            <label className="block text-[10px] uppercase font-black text-slate-400 tracking-wider font-mono">
              {t.pullTravelerWorkspace}
            </label>
            <div className="flex gap-2">
              <input
                type="text"
                required
                placeholder={t.enterRegisterUsername}
        ...
        {onInviteExternalUser && (
          ...
          >
            <label className="block text-[10px] uppercase font-black text-slate-400 tracking-wider font-mono">
              {t.addTempExternalTraveler}
            </label>
            <div className="flex gap-2">
              <input
                type="text"
                required
                placeholder={t.enterNameTemp}
                value={externalName}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-[11px] text-slate-200">
                      {p.name} {p.id === activeUserId && t.youLabel}
                    </span>
                ...
                {upgradingId === p.id && (
                  ...
                  >
                    <p className="text-slate-400 leading-normal">
                      {t.seamlessMergeHistory}
                    </p>
                    <div className="flex gap-1.5">
                      <input
                        type="text"
                        placeholder={t.registeredUsernameLabel}
